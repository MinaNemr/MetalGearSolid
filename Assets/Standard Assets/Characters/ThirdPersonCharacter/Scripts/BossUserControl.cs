			...
			if (onRange)
			{
				...
				if (Physics.Raycast (new Vector3(transform.position.x,transform.position.y+3.2f,transform.position.z), fwd, out hit, 100.0f)) {
					if (hit.transform.tag == "snake") {
						...
						Vector3 dir =new Vector3 (hit.transform.position.x, hit.transform.position.y + 3.2f, hit.transform.position.z) - new Vector3 (transform.position.x, transform.position.y + 3.3f, transform.position.z);
						Rigidbody bullet = (Rigidbody)Instantiate(projectile, bulletPlace.position, Quaternion.LookRotation(new Vector3 (hit.transform.position.x, hit.transform.position.y + 3.2f, hit.transform.position.z) - new Vector3 (transform.position.x, transform.position.y + 3.3f, transform.position.z)));
